...
export default function FaqSection() {
  ...
  return (
    <>
      <div className={`flex flex-row justify-between items-center gap-4 mb-6 ${isMobile && "relative"}`}>
        <h4 className="text-2xl md:text-3xl text-text font-medium md:text-center m-0">Frequently{isMobile && <br/>} asked questions</h4>
        <a className={`flex flex-row items-start justify-center gap-2 ${isMobile && "absolute bottom-0 right-0" }`}>
      ...
      <div className="flex flex-col md:flex-row justify-between  items-start gap-8 mb-12">
        ...
        <div className={`w-80 flex ${isMobile && "m-auto"}`}>
            <div className='mt-10 flex flex-col justify-center items-center p-5 rounded-lg border gray-5 gap-5'>
              <QuestionChat />
              <h5 className='text-lg text-text font-medium text-center'>Do you have more questions?</h5>
              <p className='text-text text-sm text-center'>Our friendly customer support team is here to chat and guide you through your learning journey.</p>
              <a className={` bg-primary text-sm w-full md:w-fit md:text-base text-white rounded-lg px-5 py-3 ${isMobile && "text-center"}`}>Ask Us Now</a>
            </div>
        </div>
      </div>
    </>
