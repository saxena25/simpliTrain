...
const CoursesByMe = ({ course, otherCourses }) => {
  const isMobile = checkIsMobile();
  const navigate = useNavigate();

  const getLowestDiscountedPrice = (batches) => {
    if (!batches || batches.length === 0) return Infinity;
  ...
  // console.log("coursesByme :", course?.instructor?.id);

  return (
    <div className="flex flex-col">
      <div className="flex flex-row justify-between items-center mb-3">
        <h5 className="text-2xl md:text-3xl font-bold ">
          Courses by {course?.instructor?.name}
        </h5>
        <button className="w-20 flex flex-row justify-between items-center" onClick={() => navigate("/courses", {state: {instructorId: course?.instructor?.id}})}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 py-5 md:py-10">
        {otherCourses?.slice(0, 3).map((item, index) => {
          // Get the lowest price for each course
          const lowestPrice = getLowestDiscountedPrice(item?.courseBatches || []);

          return (
            <Link to={`/courses/${item.id}`} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}  key={index + 1}>
              <div className={`relative`}>
                ...
                {item?.batchMode || "No Batch Mode Available"}
                </p>
            ...
            <div className={`relative`}>
              ...
              <div className="flex flex-row items-center mt-2">
                ...
                <div className="ml-2 flex flex-row gap-2">
                  ...
                  <p className="text-sm text-gray-500">
                    <span className="font-semibold">{item?.averageRating}</span> ({item?.reviewsCount})
                  </p>
                </div>
              </div>
              <div className="py-2 px-3 bg-gray-4 rounded-full mt-2 w-fit">
              ...
              <p className="mt-2 text-sm font-semibold">
                <span className="text-xs text-gray-500">Starts From</span> {lowestPrice === Infinity ? "Not Available" : `₹ ${lowestPrice}/-`}
              </p>
            </div>
          </Link>
        ))} */}
